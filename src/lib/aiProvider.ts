...
export function getDocAnalysisModel(): LanguageModel {
  if (!env.GOOGLE_API_KEY) {
    throw new Error("GOOGLE_API_KEY is required for Document Analysis (Gemini)");
  }
...
export function getRiskAssessmentModel(): LanguageModel {
  if (!env.ANTHROPIC_API_KEY) {
    throw new Error("ANTHROPIC_API_KEY is required for Risk Assessment (Claude)");
  }
 ...
 */
export function getModelForRole(role: ModelRole): { model: LanguageModel; provider: AIProvider; modelName: string } {
  switch (role) {
    case "doc_analysis":
      return {
